...
from JiaoZiDT_Web_PO.Common.dir_config import screenshot_dir
from JiaoZiDT_Web_PO.Common.logger import MyLogging
...
class BasePage:
    def __init__(self,driver: WebDriver):
        ...
        self.driver=driver
        #初始化log
        # self.logger = MyLogging('debug').my_log()
    ...
    # 7.截图
    # def save_page_shot(self, img_name):
    #         # 将图片存储到 outputs的screensshots目录下
    #         # 图片命名规范：页面名称_页面行为_时间.png
    #     now = time.strftime("%Y-%m-%d%H:%M:%S")
    #
    #     screenshot_path = screenshot_pa + "/{}_{}.png".format(img_name, now)
    #     self.driver.save_screenshot(screenshot_path)
    #     logging.info("截取当前页面图片保存在： {}".format(screenshot_path))
#图片上传
...
#6.获取元素文本
    ...
    def switch_to_iframe(self,loc):
        #等待---切换
        try:
            WebDriverWait(self.driver,20).until(expected_conditions.frame_to_be_available_and_switch_to_it(loc))
        except:
            self.logger.exception("切换iframe失败！")
            raise
